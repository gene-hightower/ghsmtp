...
class SockBufferSource : public boost::iostreams::source {
public:
  explicit SockBufferSource(int fd)
    : fd_(fd)
    , timed_out_(false)
  {
    int flags;
    PCHECK((flags = fcntl(fd_, F_GETFL, 0)) != -1);
    PCHECK(fcntl(fd_, F_SETFL, flags | O_NONBLOCK) != -1);
  }
  bool input_pending(std::chrono::milliseconds wait) const
  {
    ...
    int inputs;
    PCHECK((inputs = select(fd_ + 1, &rfds, NULL, NULL, &tv)) != -1);
  ...
  std::streamsize read(char* s, std::streamsize n)
  {
    ...
    while ((n_read = ::read(fd_, static_cast<void*>(s),
...
class SockBufferSink : public boost::iostreams::sink {
public:
  explicit SockBufferSink(int fd) : fd_(fd)
  {
  }
  std::streamsize write(const char* s, std::streamsize n)
  {
    return ::write(fd_, static_cast<const void*>(s), static_cast<size_t>(n));
  }
