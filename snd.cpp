...
DEFINE_string(service, "smtp-tor", "service name");
...
int conn(DNS::Resolver& res, Domain const& node, uint16_t port)
{
  ...
  if (use_4) {
    ...
    for (auto addr : addrs) {
      ...
      in4.sin_port   = htons(tor_port); // htons(port);
      CHECK_EQ(inet_pton(AF_INET, tor_host /* addr.c_str() */,
      ...
      LOG(INFO) << "connected via TOR to " << reply.addr() << ':'
                << reply.port() << '\n';
         ...
         std::vector<content> const& bodies)
{
  ...
  if (FLAGS_use_esmtp) {
    ...
    if (FLAGS_slow_strangle) {
      ...
      auto ehlo_str = fmt::format("EHLO mystery.tor\r\n");
