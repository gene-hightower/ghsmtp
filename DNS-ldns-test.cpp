...
int main(int argc, char const* argv[])
{
  ...
  CHECK_EQ(strcmp(std::get<RR_AAAA>(aaaaddrs_a[0]).c_str(), "2001:4860:4860::8888"), 0);

  auto aaaaddrs_b = res.get_records(RR_type::AAAA, goog_b);
  CHECK_EQ(aaaaddrs_b.size(), 1U);
  CHECK_EQ(strcmp(std::get<RR_AAAA>(aaaaddrs_b[0]).c_str(), "2001:4860:4860::8844"), 0);
  ...
  std::stable_sort(mxes.begin(), mxes.end(),
            [](RR const& a, RR const& b)
            {
              if (std::holds_alternative<RR_MX>(a) &&
                  std::holds_alternative<RR_MX>(b)) {
  ...
  auto danes = res.get_records(RR_type::TLSA, DNS::Domain("_25._tcp.digilicious.com"));
