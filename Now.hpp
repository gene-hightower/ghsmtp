...
public:
  Now()
  {
    ...
    CHECK_EQ(strftime(c_str_, sizeof c_str_, "%a, %d %b %Y %H:%M:%S %z", &my_tm), sizeof(c_str_) - 1);
  }
  time_t sec() const { return tv_.tv_sec; }
  suseconds_t usec() const { return tv_.tv_usec; }
  bool operator==(Now const& that) const { return (this->sec() == that.sec()) && (this->usec() == that.usec()); }
...
private:
  ...
  friend std::ostream& operator<<(std::ostream& s, Now const& now) { return s << now.c_str_; }
};
