...
int main(int argc, char* argv[])
{
  ...
  std::string bfr;
  std::format_to(std::back_inserter(bfr), "Message-ID: {}\r\n",
                 mid_str.c_str());
  std::format_to(std::back_inserter(bfr), "From: \"Gene Hightower\" <{}>\r\n",
                 from.as_string(Mailbox::domain_encoding::utf8));
  std::format_to(std::back_inserter(bfr), "To: \"Gene Hightower\" <{}>\r\n",
                 to.as_string(Mailbox::domain_encoding::utf8));
  std::format_to(std::back_inserter(bfr),
                 "Subject: Testing, one, two, three.\r\n");
  std::format_to(std::back_inserter(bfr), "Date: {}\r\n", date.c_str());
  std::format_to(std::back_inserter(bfr),
                 "Authentication-Results: {}; none\r\n", server_identity);
  std::format_to(std::back_inserter(bfr), "MIME-Version: 1.0\r\n");
  std::format_to(std::back_inserter(bfr),
                 "Content-Type: text/plain; charset=utf-8\r\n");

  std::format_to(std::back_inserter(bfr), "\r\n");

  std::format_to(std::back_inserter(bfr), "This is the body of the email.\r\n");
